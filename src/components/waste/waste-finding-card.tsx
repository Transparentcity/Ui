...
export function WasteFindingCard({
  ...
  const handleAskSeymour = (e: React.MouseEvent) => {
    ...
    const prompt = encodeURIComponent(
      `Analyze this waste detection finding in detail:\n\n` +
      `Finding: ${finding.id} — ${finding.subcategory}\n` +
      `Entity: ${finding.entity}\n` +
      `Metric: ${finding.metric} ${finding.metricDetail}\n` +
      `Amount: ${finding.amount != null ? formatDollar(finding.amount) : "N/A"}\n` +
      `Description: ${finding.description}\n` +
      `Detection method: ${finding.tool}\n\n` +
      `Please explain:\n` +
      `1. What this means and why it matters\n` +
      `2. What could legitimately explain this anomaly\n` +
      `3. What specific actions I should take to investigate\n` +
      `4. Who should be notified or involved`
    )
    router.push(`/compose?prefill=${prompt}`)
  }
